...
### 6.2 实验 5：稀疏终局奖励 + 轻量有界 Shaping (当前最高优先级)

**背景与动机**：
经过实验 4 和 4.1 的验证，我们发现原论文的 $1/x$ 奖励公式存在致命的“局部最优黑洞”。即使在近距离（0.3m~0.6m）下，Agent 依然会选择停在 1.1m 处刷取极高的轨迹对齐分（每步约 25 分），而不愿意为了靠近托盘（增加 5 分）去承担破坏轨迹对齐（损失 9 分）和撞击托盘（扣 100 分）的风险。

**核心假设**：
只要使用无上界的 $1/x$ 公式，Agent 总能找到一个不完成任务但收益极高的“舒适区”。必须彻底抛弃 $1/x$，改用有界的指数函数 $exp(-x/\sigma)$，将每步 Shaping 奖励严格限制在 `[0, 1]` 之间，同时大幅提高终局奖励，让“完成任务”成为数学上唯一的全局最优解。
...
2. **放大终局奖励**：
   * 到达奖励 `rg` 从 10 提高到 **200**。
   * 保持推盘惩罚 `rp = -100`，确保推盘收益远小于干净插入。
3. **保留近距离课程**：
   * 继续在 `0.3m ~ 0.6m` 出生（Stage A），确保 Agent 能轻易通过随机探索拿到 200 分的终局奖励，从而明白任务的真正目标。
4. **快速验证**：只跑 500-1000 代。

**预期结果与应对方式**：
*   **如果 `push_free_success_rate` 快速上升**：证明“有界 Shaping + 大终局 + 近距课程”是正确的解法组合！下一步将进入 Stage B，逐步扩大出生距离。
*   **如果依然失败（比如乱撞或停滞）**：说明即使在明确的终局奖励指引下，Agent 依然无法学会在最后 0.5m 内精细控制。这可能意味着我们需要重新审视动作空间的映射（比如速度缩放比例）或者观测信息的有效性。

---

### 6.3 实验 5.1：消除探索恐惧 (当前最高优先级)

**背景与动机**：
实验 5 证明了有界 Shaping 成功消除了“刷分黑洞”，但 Agent 依然停在门外不敢进去。深入分析发现，这是因为 `-100` 的推盘惩罚在初期像一堵无形的墙，导致了“探索瘫痪”。Agent 为了避免极大概率的碰撞扣分，宁愿放弃靠近的奖励。同时，它至今没有一次偶然触发过 `rg`，所以它根本不知道前方有 200 分的大奖。

**核心假设**：
在初期，必须大幅降低探索成本（降低惩罚），并强行把它“喂”到嘴边（收窄初始状态），让它体验到一次 200 分的快感。等它学会了“插入能拿大奖”后，再在后续课程中慢慢提高推盘惩罚，逼它“干净地插入”。

**实验计划**：
1. **极大降低推盘惩罚（消除恐惧）**：将 `alpha_5` (推盘惩罚) 从 `100.0` 暴降到 `1.0`。
2. **进一步收窄初始随机范围（消除无知）**：
   * 将 Stage A 的初始偏角从 `±10°` 降到 `±2°`。
   * 将横向偏差从 `±0.15m` 降到 `±0.05m`。
3. **快速验证**：只跑 500-1000 代。

**预期结果与应对方式**：
*   **如果 `push_free_success_rate` 快速上升**：证明“消除探索恐惧”是正确的解法！下一步将进入 Stage B，逐步扩大出生距离，并慢慢恢复推盘惩罚。
*   **如果依然失败**：说明即使在极低的惩罚和极近的距离下，Agent 依然无法学会在最后 0.5m 内精细控制。这可能意味着我们需要重新审视动作空间的映射（比如速度缩放比例）或者观测信息的有效性。
