...
基于 3.1、3.2、3.3 的连贯实验，我们得出了非常清晰的结论：

1. **轨迹走廊 (3.1) 是有效的**：它成功规范了远场接近行为，消除了“乱走”和“远场推盘”。
2. **近场 Commit 奖励 (3.2) 是必须的**：没有它，智能体会陷入“门前停滞”的局部最优。它成功激发了智能体的插入意愿。
3. **当前的瓶颈是“死区作弊 (Reward Hacking)”**：在 3.3 中，尽管我们加重了推盘惩罚，智能体依然选择“斜着浅插推着走”。因为它发现，只要保持浅插，就能持续获得 `r_commit_insert`，而这个收益大于推盘的惩罚。
4. **下一步核心**：必须在死区（插歪了）**熔断** Commit 奖励，并鼓励撤退重试（实验 3.4）。
