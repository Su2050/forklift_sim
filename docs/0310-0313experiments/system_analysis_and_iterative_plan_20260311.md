...
### 6.1 实验 4.1：近距离课程验证 (当前最高优先级)

**背景与动机**：
实验 4 证明了在 3.5m 的大空间内，原论文的 $1/x$ 奖励会导致严重的数值爆炸和“木头人/倒车逃跑”等 Reward Hacking 行为。但原论文的实际工作空间仅为 1.8m 的三角区。我们假设：**$1/x$ 公式本身没问题，问题出在我们的初始距离太远，导致 Agent 无法通过随机探索偶然发现“插入成功”的终局大奖，只能被迫刷取每步的 Shaping 奖励。**

**重大 Bug 发现与修复**：
在准备实验 4.1 时，我们发现了一个导致实验 4 绝对不可能成功的致命几何 Bug：
*   我们的托盘经过了 1.8 倍缩放，深度达到了 `2.16m`。
*   我们的叉臂长度只有 `1.2m`。
*   之前实验 4 中定义的 `dist_center` 是“叉臂中心到托盘中心的距离”。但由于叉臂比托盘短得多，即使完全插入（叉根抵住托盘前沿），叉臂中心距离托盘中心依然有 `0.48m` 的物理极限距离！
*   而到达奖励 `rg` 的触发阈值是 `dist_center < 0.1m`，这意味着 **Agent 在物理上永远无法触发到达奖励 `rg`**，它永远拿不到那 10 分的终局奖励，只能靠倒车刷轨迹分！
*   **修复方案**：将 `dist_center` 重新定义为“叉臂中心到**目标叉臂中心**的距离”（目标叉臂中心 = 托盘前沿向内 0.6m 处）。

**实验计划**：
1. **修复 `dist_center` 几何计算**：如上所述，修正目标点。
2. **修改出生点 (Spawn Range)**：将叉车的初始位置缩短，使得叉尖距离托盘前沿在 `0.3m ~ 1.0m` 之间（即 Stage A 近距离课程）。
   * `x_min = -2.5`, `x_max = -1.8` (对应叉尖距离 0.25m ~ 0.95m)
   * `y` 偏差 `±0.15m`，`yaw` 偏差 `±10°`。
3. **保持 Reward 不变**：保留原汁原味的 $1/x$ 奖励和现有的权重，不加任何复杂的条件惩罚。
4. **快速验证**：只跑 500-1000 代。

**预期结果与应对方式**：
*   **如果 `push_free_success_rate` 突破 0 并达到 30% 以上**：证明假设成立！论文的 Reward 设计在近距离下是完全有效的，之前的失败是因为距离过远+几何计算Bug。下一步将进入 Stage B，逐步扩大出生距离。
*   **如果成功率依然为 0**：说明即使在论文的尺度下该 Reward 也不 work。我们将需要重新审视物理环境或考虑更换 Reward 形式（如使用有界函数 `exp`）。
