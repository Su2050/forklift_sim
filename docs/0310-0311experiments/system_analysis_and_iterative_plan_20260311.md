...
## 9. 实验 2A：先在不改 RL 的前提下，提升预训练数据分布

### 假设 H2A

当前预训练失败的主因，不一定只是模型容量，而是数据分布不够贴近“最后几厘米”的难点。  
如果扩大数据集并显式提升近场/硬例比例，预训练误差会明显下降。
...
- 若离线指标明显改善，再进入对应 RL 验证。
- 若数据量增加后收益不大，说明监督目标本身太粗。

---

## 10. 实验 2B：升级预训练目标，从粗位姿回归变成更密集的几何监督

### 假设 H2B

当前 `(x, y, yaw)` 粗回归不够支撑最后几厘米的精对齐。  
如果引入孔位/开口关键点、heatmap 或多任务监督，视觉 backbone 会学到更稳定的近场几何特征。
...
1. `pose regression`
2. `pose regression + slot keypoints`
3. `pose regression + local heatmap`
...
- 近场子集 `Y MAE`
- 近场子集 `Yaw MAE`
- 后续 frozen RL 的 push-free 插入率

### 决策规则

- 只要多任务监督明显降低近场误差，优先保留它，再谈更大 backbone。

---

## 11. 实验 2C：在监督目标和数据分布固定后，比较 backbone 容量

### 假设 H2C

如果数据分布和监督目标都已经更对了，但误差仍高，瓶颈才更可能在 backbone 容量。
...
- 若 `ResNet18` 明显优于 MobileNet，才考虑切主线 backbone。
- 若提升不大，则说明瓶颈不在 backbone 型号。

---

## 12. 实验 3：在“更准的预训练”前提下，重新验证全程冻结 RL

### 假设 H3

如果预训练已经足够准，那么 RL 根本不需要负责“修眼睛”，只需学习控制。  
此时**全程冻结 backbone**反而可能是最稳的方案。

### 只改一个因素

- 使用改进后的最佳预训练权重
- Stage 1 改为 2 动作 `approach-only`
- 全程冻结 backbone
...
- 如果 frozen RL 已经能达到 `push_free_success_rate >= 0.40`，则说明“RL 修视觉”这条线可以正式降级。

---

## 13. 实验 4：相机视角 ablation，向论文的侧视近场几何靠拢
...
- 当前最危险的问题不是没有实验，而是还在被“假成功”误导。
- 不先把 `push_free_success_rate` 建起来，后面很难判断到底有没有进步。
...
- 这是最直接向论文靠拢的一步。
- 当前 Stage 1 success 已不要求 lift，但动作里仍有 `lift`，这是明显的结构性噪声源。
- 这是低成本、强解释力、单因素清晰的实验。
...
- 现有证据已经很强地说明：**靠 RL 微调修视觉误差不可靠**。
- 所以下一阶段的主战场应该回到**预训练质量本身**。
- 其中比“直接换 ResNet”更优先的是：
  - 数据分布
  - 监督目标密度
...
当前要想接近论文的成功路径，**不是先纠结 ResNet18 还是 MobileNet，而是先把问题拆对**：

1. **先把 approach-only 训练做干净**
2. **把“真成功”与“推土机假成功”区分开**
3. **把视觉误差尽量在预训练阶段解决，而不是指望 RL 端到端去补**
4. **再像论文一样，把 lift / decision 单独拆出来**

只有这样，才有机会从“能靠近”真正走到“能稳定插入并最终完成任务”。
