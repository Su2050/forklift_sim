...
## 11. 实验 3：相机视角 ablation，向论文的侧视近场几何靠拢
...
### 建议 1：先做实验 0 和 实验 1 (已启动)

原因：
- 当前最危险的问题不是没有实验，而是还在被“假成功”误导。
- 实验 1 (2动作 approach-only) 是最直接向论文靠拢的一步，消除了不必要的探索噪声。

### 建议 2：立刻做实验 2 (RRL 范式验证)

原因：
- 根据对论文的重新审视，我们可能一直在“解错题”。
- 论文并未进行特定任务的坐标回归预训练，而是直接使用了冻结的 ImageNet 特征 (RRL 范式)。
- 我们应该立刻用 `ResNet-18` (ImageNet 权重，全程冻结) 跑一组对照实验，将“理解物理含义”的任务完全交给下游的 FC 层。

### 建议 3：暂缓特定任务预训练的数据收集

原因：
- 如果实验 2 (RRL 范式) 能够成功，说明我们根本不需要去死磕“预训练 Y 误差 16cm”的问题。
- 只有在 RRL 范式彻底失败后，我们才需要回到备用路线，去收集更大、更密集监督的数据集。
