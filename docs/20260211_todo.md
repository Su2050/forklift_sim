...
## 五、C 类对比总结（暂定，待 C2 完成后更新）

| 变体 | `frac_lifted` | `frac_success` | `success_now` | `grace_zone_frac` | 结论 |
|------|:---:|:---:|:---:|:---:|------|
| S1.0N (2000 iter) | 1.66% | 0.49% | 1.46% | ~0.8% | 基线 |
| C1 (600 iter) | ~1.5% | 0.10%~0.29% | 0.59%~1.76% | 0.68%~1.46% | 中性偏弱，待组合验证 |
| C2 (600 iter) | — | — | — | — | **运行中** |
...
| 7 | C2（越界衰减） | `20260211_201230_train_s1.0o_C2_s42.log` | **运行中** |

---

## 七、Round 2 计划（待 C2 完成后执行）

1. **各类赢家**：
   - A 类赢家：**A3**（premature 分段温和化 + lift delta 势函数）
   - B 类赢家：**B1**（增大 sigma_y/sigma_yaw + k_hold_align）
   - C 类赢家：C1 vs C2（待 C2 完成后对比 `frac_success / frac_success_now` 转化率）

2. **创建组合分支并训练**（800-1200 iter）：
   - `exp/DO-O-A3B1`：A3 + B1
   - `exp/DO-O-A3C*`：A3 + C 类赢家
   - `exp/DO-O-A3B1C*`：全组合（最终候选）
