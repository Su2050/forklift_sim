...
MD_FILE="/home/uniubi/projects/forklift_sim/docs/0310-0314experiments/exp5_7_auto_analysis.md"

echo "# Exp 5.7 自动分析报告 (每10分钟更新)" > "$MD_FILE"
...
while true; do
    ...
    if [ -f "$LOG_FILE" ]; then
        ...
        if (( $(echo "$RG_AVG > 0.3" | bc -l) )); then
            echo "- **结论**: 🚀 破冰成功！重赏之下必有勇夫！" >> "$MD_FILE"
        elif (( $(echo "$RG_AVG > 0" | bc -l) )); then
            echo "- **结论**: ⏳ 开始有突破了，继续观察。" >> "$MD_FILE"
        else
            echo "- **结论**: ❌ 依然是 0，5000 分的诱惑还不够？或者需要加时间惩罚。" >> "$MD_FILE"
        fi
        echo "---" >> "$MD_FILE"
    fi
done
